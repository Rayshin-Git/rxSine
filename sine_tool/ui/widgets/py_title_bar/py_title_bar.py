...
class PyTitleBar(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        self._default_btns = default_btns
        ...
        # ADD Buttons
        if is_custom_title_bar:
            for c, b in zip(self._default_btns,
                            [self.minimize_button, self.maximize_restore_button, self.close_button]):
                if c:
                    self.bg_layout.addWidget(b)
    ...
    def maximize_restore(self, e=None):
        ...
        def change_ui():
            if _is_maximized:
                # self._parent.main_ui.central_widget_layout.setContentsMargins(0, 0, 0, 0)
                # self._parent.main_ui.window.set_stylesheet(border_radius=0, border_size=0)
                self.maximize_restore_button.set_icon(
                    Functions.set_svg_icon("icon_restore.svg")
                )
            else:
                # self._parent.main_ui.central_widget_layout.setContentsMargins(10, 10, 10, 10)
                # self._parent.main_ui.window.set_stylesheet(border_radius=10, border_size=2)
                self.maximize_restore_button.set_icon(
        ...
        if self._parent.isMaximized():
            ...
            _is_maximized = True
            _old_size = QtCore.QSize(self._parent.width(), self._parent.height())
    ...
    def setup_ui(self):
        ...
        # self.logo_svg = QtWidgets.QSvgWidget()
